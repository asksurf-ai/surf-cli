...
export function useInfiniteWalletLabelsBatch(params: Omit<WalletLabelsBatchParams, 'offset'>) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) => proxyGet<ApiResponse<WalletLabelsBatchItem>>('wallet/labels/batch', { ...params!, offset: String(pageParam) }),
    initialPageParam: 0,
    getNextPageParam: (last) => {
      const m = last?.meta;
      if (!m?.total || !m?.limit) return undefined;
...
export function useInfiniteWalletNetWorth(params: Omit<WalletNetWorthParams, 'offset'>) {
  return useInfiniteQuery({
    ...
    queryFn: ({ pageParam = 0 }) => proxyGet<ApiResponse<WalletNetWorthItem>>('wallet/net-worth', { ...params!, offset: String(pageParam) }),
    initialPageParam: 0,
    getNextPageParam: (last) => {
      const m = last?.meta;
      if (!m?.total || !m?.limit) return undefined;
